...
GameView::GameView(BoardController &board, PlayerController &playerController)
    :   m_boardController{ board },
        ...
        m_pairSelector{ { 110, 110 }, {780, 300}, 50 },
        m_statusText{ RenderWrapper::createString("", {600, 50}, 20, sf::Color::White) }
{}
...
GameView::setup() {
    ...
    m_rollDiceBtn.addActionListener([this](const sf::Vector2i &cord) {
        ...
        m_playerController.enqueueAction(PlayerAction::ROLL_DICE);
    });
    ...
    m_commitSelectionBtn.addActionListener([this](const sf::Vector2i &cord) {
        ...
        m_playerController.enqueueAction(PlayerAction::MAKE_SELECTION);
    });

    m_switchPlayerBtn.addActionListener([this](const sf::Vector2i &cord) {
        m_playerController.enqueueAction(PlayerAction::SWITCH_PLAYER);
    });
}
